...
-- Returns the bricks entry with one brick locked and another one with a key
function LevelMaker.addKeyLocked(bricks)
    local keyIndex = math.random(1, #bricks)
    local lockedIndex = math.random(1, #bricks)

    if keyIndex == lockedIndex then
        return LevelMaker.addKeyLocked(bricks)
    end

    bricks[lockedIndex].locked = true
    bricks[keyIndex].hasKey = true
